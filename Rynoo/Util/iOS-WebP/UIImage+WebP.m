...
+ (UIImage *)imageWithWebPData:(NSData *)imgData{
   ...
   // RLogs(@"WebP decoder version: %d", rc);
    ...
    RLogs(@"result img size - %@", NSStringFromCGSize(result.size));
    return result;
}

- (NSData *)dataWebPWithQuality:(float)quality{
 //   int rc = WebPGetEncoderVersion();
 //   RLogs(@"WebP encoder version: %d", rc);
    
    CGImageRef imageRef = self.CGImage;
    CGColorSpaceRef colorSpace = CGImageGetColorSpace(imageRef);
    if (CGColorSpaceGetModel(colorSpace) != kCGColorSpaceModelRGB) {
        RLogs(@"Sorry, we need RGB");
    }
    ...
    }else
    {
        ...
        ret_size = WebPEncodeRGBA(rawData, width, height, stride, quality, &output);
    }
    
    RLogs(@"++++After WebEncode");
...
- (BOOL)writeWebPLosslessToFilePath:(NSString *)filePath{
    ...
    return [self writeWebPToFilePath:filePath quality:kWebPLossless];
}
...
+(BOOL)writeWebpData:(NSData*)webpdata toFilePath:(NSString*)filePath
{
   // NSString *fileName = [filePath lastPathComponent];
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:folderName]) {
        ...
        if ([[NSFileManager defaultManager] createDirectoryAtPath:folderName withIntermediateDirectories:YES attributes:nil error:nil]){
        }
    }
   ...
   BOOL success =  [webpdata writeToFile:filePath options:NSDataWritingAtomic error:&error];

    
  // BOOL success = [webpdata writeToFile:filePath options:0 error:&error];
    if (!success) {
        RLogs(@"writeToFile failed with error %@", error);
    }
