...
#define MENU_IMAGE @"menu-icon@3x"
...
+ (SlideNavigationController *)sharedInstance
{
	if (!singletonInstance)
		RLogs(@"SlideNavigationController has not been initialized. Either place one in your storyboard or initialize one in code");
...
{
	if (singletonInstance)
		RLogs(@"Singleton instance already exists. You can only instantiate one instance of SlideNavigationController. This could cause major issues");
...
- (void)switchToViewController:(UIViewController *)viewController withSlideOutAnimation:(BOOL)slideOutAnimation popType:(PopType)poptype andCompletion:(void (^)())completion
{
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    	...
    	if ([self shouldDisplayMenu:MenuLeft forViewController:viewController])
        {
            ...
            [btnBack setImage:[UIImage imageNamed:@"menu-icon@3x.png"] forState:UIControlStateNormal];
            btnBack.frame = CGRectMake(0, 0, 23, 16);
            [btnBack addTarget:self action:@selector(leftMenuSelected:) forControlEvents:UIControlEventTouchUpInside];
